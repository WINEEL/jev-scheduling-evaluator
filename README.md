...
narrow questions about each, and turns the probabilities that come back into
...
**Soft constraints do not.** Is this a *fair* way to spread the work? Has
anybody been leaned on too heavily? Were people's stated preferences
respected well enough to send this out? There is no threshold that settles
those, and the usual response is to invent one — a fairness score, a weighted
sum, a magic number in a config file that nobody can defend. They are questions
about a situation, and reading a situation is what a language model is for.
...
backend/app/
  soft_constraints/
    state.py        normalized input + the exact arithmetic, computed in Python
    scenarios.py    the three invented drafts — the only input the API accepts
    questions.py    the five typed Jev questions
    evaluator.py    the one seam that talks to TypeSafe, and answer parsing
    results.py      typed results, so callers never parse model output
    policy.py       probabilities → status. Imports no SDK, asks no model.
  api/              two endpoints over the three scenarios
  main.py           the server

frontend/src/
  app/page.tsx      the whole UI: three panels, in the order the argument runs
  app/api/jev-demo/ a same-origin proxy, so the API key stays server-side
  lib/              transport, types, and the presentation rules (all tested)
```
...
- **Three scenarios is not an evaluation set.** They were written to be
  legible, not representative, and one of them (`attention`) has not reliably
  appeared in live runs at all.
